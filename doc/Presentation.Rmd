...
```{r}
#ggplotly(depth_dist)
cowplot::plot_grid(depth_dist, latrine_dist, align = "h", ncol = 2, rel_heights = c(0.50, 0.50))
...
```{r, include=FALSE}
source(file = "/cloud/project/R/04_analysis_heatmap_correlation.R",local = knitr::knit_global())
```
...
```{r, include=FALSE}
source(file = "/cloud/project/R/04_analysis_ii.R",local = knitr::knit_global())
```

```{r}
ggplotly(pca_arrow_points)
```
---
```{r}
ggplotly(pca_dist)
